...
repository-code: "https://github.com/jamilhanouneh/BrainGraphNet"
url: "https://github.com/jamilhanouneh/BrainGraphNet"
abstract: "A production-ready deep learning framework for modeling temporal evolution of functional brain connectivity using EvolveGCN. This project predicts future brain connectivity patterns from fMRI time series data, enabling applications in neurodegenerative disease detection."
...
```markdown
# Contributing to BrainGraphNet

First, thank you for considering contributing to BrainGraphNet! It's people like you that make this project such a great tool for brain connectivity research.
...
def process_connectivity_matrix(
    ...
    Args:
        connectivity: Input connectivity matrix (N, N)
        threshold: Threshold value between 0 and 1
    
    Returns:
        Thresholded connectivity matrix (N, N)
    
    Raises:
        ValueError: If threshold not in[1]
    """
    if not 0 <= threshold <= 1:
        raise ValueError(f"Threshold must be in, got {threshold}")[1]
...
Example:
```
feat(models): add attention mechanism to EvolveGCN

Implemented multi-head attention for improved temporal modeling.
Achieves 0.5% improvement in R² score.
...
- Credited in academic publications (for major contributions)
...
Thank you for contributing to BrainGraphNet! 🧠
```
...
All notable changes to BrainGraphNet are documented in this file.
...
#### Models
- EvolveGCN implementation with temporal weight evolution
- Temporal GCN alternative architecture
- Graph convolution layers (GCN, GAT)
- Custom loss functions (MSE + correlation + temporal consistency)
- Evaluation metrics (MSE, MAE, R², correlation)

#### Data Processing
- Synthetic brain connectivity data generator
- HCP Aging data loader and processor
- Graph builder (connectivity matrices → temporal graphs)
- PyTorch Dataset and DataLoader implementations
- Support for custom connectivity data
...
- Brain atlas definitions (AAL, Gordon333)
...
- Achieved R² score of 0.4904 on test set
- Achieved Pearson correlation of 0.7386
- Model trained on 50 HCP Aging subjects with 349 brain regions
- 125.9 million parameters
- ~15 minutes training on CPU
...
- Complete README with badges and results
...
[1](https://ppl-ai-file-upload.s3.amazonaws.com/web/direct-files/attachments/images/22428755/88a738be-e1eb-4299-be47-57ccee3048fd/05_metrics_summary.jpg)
